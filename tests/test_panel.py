...
def test_stock_panel_in_cio_tools():
    """'stock_panel' must be in CIO_TOOLS and the total count must be 25."""
    ...
    assert len(agent_mod.CIO_TOOLS) == 25, (
        f"Expected 25 tools in CIO_TOOLS, got {len(agent_mod.CIO_TOOLS)}: {tool_names}"
    )
