...
class TestResolveChain:
    ...
    def test_single_service_role_is_one_link(self):
        ...
        assert len(chain) == 1
        assert chain[0]["service"] == "claude"
        assert "daily_limit" not in chain[0]

    def test_unknown_role_falls_back_to_default(self):
        """resolve_chain for an unknown role → 1 default (claude) link."""
        from cio.committee.models import resolve_chain
...
class TestChainSelection:
    ...
    def test_specialist_single_link_uses_claude(self, monkeypatch):
        """A specialist role (e.g. market) dispatches to claude via its 1-link chain."""
