...
def find_swings(data: pd.DataFrame, n: int = 2) -> pd.DataFrame:
    ...
    for i, swing in enumerate(swings):
        if swing < 0:
            ...
            highs.append(0)
            lows.append(np.nanmin(low_data[i - n + 1 : i + 1]))
        elif swing > 0:
            # Up swing, find high price
            highs.append(np.nanmax(high_data[i - n + 1 : i + 1]))
            lows.append(0)
        else:
            # Price movement
            highs.append(0)
    ...
    swing_df.loc[
        swing_df["Last"] == 0,
        :
    ] = np.nan  # Remove non-swing data

    return swing_df


def find_data_swings(data: pd.Series, n: int = 2) -> pd.DataFrame:
    """Locates swings in the inputted data using a moving average gradient
    method.
    ...
    # Determine last swing
    trend = rolling_signal_list(-swings)
    swings_list = merge_values(lows, highs)
    last_swing = rolling_value_list(swings_list)

    # Need to return both a last swing low and last swing high list
    last_low = rolling_value_list(lows)
    last_high = rolling_value_list(highs)

    swing_df = pd.DataFrame(
        data={"Highs": last_high, "Lows": last_low, "Last": last_swing, "Trend": trend},
        index=swing_data.index,
    )
    # swing_df.loc[swing_df["Last"] == 0, :] = np.nan  # Remove non-swing data
...
def classify_swings(swing_df: pd.DataFrame, tol: int = 0) -> pd.DataFrame:
    ...
    low_change = np.sign(swing_df.FSL) * (
        swing_df.FSL
        - swing_df.Strong_lows.replace(to_replace=0, value=np.nan).ffill().shift()
    )
    high_change = np.sign(swing_df.FSH) * (
        swing_df.FSH
        - swing_df.Strong_highs.replace(to_replace=0, value=np.nan).ffill().shift()
    )
    ...
    if method == 0:
        ...
        if True:
            ...
            price_at_indi_lows_change = np.sign(price_at_indi_lows) * (
                price_at_indi_lows
                - price_at_indi_lows.replace(to_replace=0, value=np.nan).ffill().shift()
            )
            price_at_indi_highs_change = np.sign(price_at_indi_highs) * (
                price_at_indi_highs
                - price_at_indi_highs.replace(to_replace=0, value=np.nan).ffill().shift()
            )
...
def candles_between_crosses(
    ...
    # Positional access: a pd.Series here carries a DatetimeIndex, so crosses[i]
    # would be a label lookup (KeyError under pandas >= 3.0). Iterate over values.
    _vals = crosses.to_numpy() if isinstance(crosses, pd.Series) else crosses
...
def rolling_value_list(values: Union[list, pd.Series]) -> list:
    """
    Calculate the rolling values for a given list or pandas Series.

    Parameters:
    values (Union[list, pd.Series]): The input values for which rolling values need to be calculated.

    Returns:
    list: The list of rolling values.

    """
    if isinstance(values, list):
        rolling_values = [values[0]]
    else:
        rolling_values = [values.iloc[0]]
    last_value = rolling_values[0]

    if isinstance(values, list):
        for i in range(1, len(values)):
            if np.isnan(values[i]) == False:
                last_value = values[i]
            rolling_values.append(last_value)
    else:
        for i in range(1, len(values)):
            if np.isnan(values.iloc[i]) == False:
                last_value = values.iloc[i]
            rolling_values.append(last_value)

    if isinstance(values, pd.Series):
        rolling_values = pd.Series(data=rolling_values, index=values.index)
...
def unroll_signal_list(signals: Union[list, pd.Series]) -> np.array:
    ...
    # Positional access: a pd.Series carries a DatetimeIndex, so signals[i] would be
    # a label lookup (KeyError under pandas >= 3.0). Iterate over values instead.
    _vals = signals.to_numpy() if isinstance(signals, pd.Series) else signals
    for i in range(1, len(_vals)):
        if _vals[i] != _vals[i - 1]:
            unrolled_signals[i] = _vals[i]
...
def merge_values(value_1: list, value_2: list) -> list:
    """
    Merge two lists of values by replacing non-null values in value_1 with corresponding values from value_2.

    Args:
        value_1 (list): The first list of values.
        value_2 (list): The second list of values.

    Returns:
        list: The merged list of values.
    """
    merged_value_list = value_1.copy()
    for i in range(len(value_1)):
        if np.isnan(value_2[i]) == False:
            merged_value_list[i] = value_2[i]

    return merged_value_list
